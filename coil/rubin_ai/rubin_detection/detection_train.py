...
def main():
    ...
    ap.add_argument("--project", default="Druns/detect", help="Ultralytics project dir")
    ap.add_argument("--name",    default="train",       help="Run name")
    args = ap.parse_args()

    model = YOLO(args.model)  # detection weights
    model.train(
        data=args.data,
        epochs=args.epochs,
        imgsz=args.imgsz,
        ...
        name=args.name,
        exist_ok=True,
        patience=50,      # early stop if plateaus
        cos_lr=True,      # nice default LR schedule
